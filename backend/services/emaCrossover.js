/**
 * EMA 10 / EMA 20 crossover strategy (1H).
 * BUY when EMA10 crosses above EMA20 and EMA20 is above EMA50; SELL when EMA10 crosses below EMA20.
 */
...
/**
 * Evaluate EMA 10/20 crossover on the last bar.
...
export function evaluate(closes) {
  ...
  if (!Array.isArray(closes) || closes.length < EMA_FILTER + 1) return empty;

  const ema10 = emaSeries(closes, EMA_FAST);
  const ema20 = emaSeries(closes, EMA_SLOW);
  const ema50 = emaSeries(closes, EMA_FILTER);
  ...
  const ema20Above50 = Number.isFinite(e50) && e20 > e50;

  if (crossUp && ema20Above50) {
    return {
      signal: 'BUY',
      entryPrice: close,
      explanation: `EMA 10 crossed above EMA 20, EMA 20 > EMA 50. Close ${close?.toFixed(2)}, EMA10 ${e10?.toFixed(2)}, EMA20 ${e20?.toFixed(2)}, EMA50 ${e50?.toFixed(2)}.`,
    };
  }
  if (crossDown) {
    return {
